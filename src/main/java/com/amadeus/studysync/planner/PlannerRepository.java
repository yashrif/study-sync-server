...
public interface PlannerRepository extends JpaRepository<Planner, UUID> {
    ...
    @Query(value = "select * from planner i where i.created_by = :id", nativeQuery = true)
    List<Planner> findAllByUser(@Param("id") UUID id);

    @Query(value = "select * from planner i where i.created_by = :id and i.id = :plannerId", nativeQuery = true)
